% Detect & Recognize Text in Images
...
boundingBox = vertcat(mserStats.BoundingBox);
w = boundingBox(:,3);
h = boundingBox(:,4);
aspectRatio = w./h;
...
boundingBoxes = vertcat(mserStats.BoundingBox);

% Convert from the [x y width height] bounding box format to the [xmin ymin xmax ymax] format for convenience.
xmin = boundingBoxes(:,1);
ymin = boundingBoxes(:,2);
xmax = xmin + boundingBoxes(:,3) - 1;
ymax = ymin + boundingBoxes(:,4) - 1;
...
textBBoxes = [xmin ymin xmax-xmin+2 ymax-ymin+2];
...
ocrtxt = ocr(Image, textBBoxes);
resultText = ocrtxt.Text;
[resultText]

% Text to speech
NET.addAssembly('System.Speech');
speakFunction = System.Speech.Synthesis.SpeechSynthesizer;
speakFunction.Volume = 100; %cant be bigger than 100
Speak(speakFunction,resultText);
